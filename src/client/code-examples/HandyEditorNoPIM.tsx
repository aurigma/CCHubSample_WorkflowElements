...
import { getStringArrayParam, getStringParam } from "../shared/code-example-params.js";
import { getErrorMessage } from "../shared/errors.js";

const HandyEditorNoPIM = ({ codeExample }: CodeExamplePageProps) => {
    ...
    /*
        ...
        In this sample, these values are read from the code example configuration.
    ...
    useEffect(() => {
        ...
        if (isScriptLoaded) {
            (async () => {
                ...
                const editorMockupIds = getStringArrayParam(codeExample, "editorMockupIds");
                const previewMockupIds = getStringArrayParam(codeExample, "previewMockupIds");

                if (!publicDesignId) {
                    console.error("No design template for this sample is specified. Ensure that you have set the publicDesignId code example param!");
                }

                if (editorMockupIds.length == 0) {
                    console.warn("No editor mockups are specified. The editor will load a template without a product visualization. If you expect to visualize a design on a mockup, ensure that you have specified the editorMockupIds code example param.");
                }
